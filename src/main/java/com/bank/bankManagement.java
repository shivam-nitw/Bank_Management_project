...
public class bankManagement { // these class provides all
	...
	createAccount(int acc,String name,
				int passCode) // create account function
	{
		try {
			// validation
			if (acc == NULL || name == "" || passCode == NULL) {
				System.out.println("All Field Required!");
				return false;
			}
			// query
			Statement st = con.createStatement();
			sql = "INSERT INTO customer(ac_no,cname,balance,pass_code) values(" + acc + ",'"
				+ name + "',1000," + passCode + ")";
...
//		}
						...
						if (ch == 1) {
							...
							if (bankManagement
									.depositMoney(
										...
										amt)) {
								System.out.println(
									"MSG : Money Deosited Successfully!\n");
							}
							else {
								System.out.println(
									"ERR : Failed!\n");
						...
						if (ch == 2) {
							...
							if (bankManagement
									.withdrawMoney(
										...
										amt)) {
								System.out.println(
									"MSG : Money Withdrawl Successfully!\n");
							}
							else {
								System.out.println(
									"ERR : Failed!\n");
	...
	public static boolean depositMoney(int sender_ac,int amount)
		throws SQLException // transfer money method
		{
		// validation
		if (sender_ac == NULL || amount == NULL) {
		System.out.println("All Field Required!");
		return false;
		}
		try {
...
//		}
	...
	public static boolean withdrawMoney(int sender_ac,
			int amount)
	throws SQLException // withdraw money method
	{
	// validation
	if (sender_ac == NULL || amount == NULL) {
	System.out.println("All Field Required!");
